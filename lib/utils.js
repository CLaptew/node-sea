...
export const get_current_platform = () => {
 ...
 */
export async function get_node_executable({ useSystemNode, nodeVersion, arch, platform, withIntl }
  = {
    useSystemNode: true,
  ...
  const node_executable_filename = `node-${platform}-${arch}-${nodeVersion}-with-intl-${withIntl}${platform === 'windows' ? '.exe' : ''}`
