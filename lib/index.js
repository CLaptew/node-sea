...
import { is_directory_exists, is_file_exists, spinner_log, get_node_executable, get_current_platform } from "./utils.js";
import ora from 'ora';
...
export default async function sea(
  script_entry_path,
  executable_path,
  { disableExperimentalSEAWarning, useSnapshot, useCodeCache, useSystemNode, nodeVersion, withIntl, arch, platform } = {
  ...
  if (!await is_directory_exists(dirname(executable_path))) {
    console.warn(`Executable directory ${dirname(executable_path)} does not exist. creating new`);
    await mkdir(dirname(executable_path), { recursive: true })
  }
